...
export default function App() {
  ...
  // YouTube Content-Relevance Engine
  const [ytApiKey, setYtApiKey] = useState('');
  const [videoData, setVideoData] = useState({}); // { [day]: { status, videoId, title, thumbnail, watchUrl, searchUrl } }
  ...
  // Builds a content-focused search query for a given roadmap day.
  // Deliberately does NOT include any view-count / trending signal —
  // relevance to the topic is the only thing that matters here.
  const buildContentQuery = (item) => {
    const topicPart = item.type === 'revision'
      ? `${course} revision recap`
      : `${course} ${item.label.replace(/[🔄✨]/g, '').trim()}`;
    return `${topicPart} tutorial`.trim();
  };

  // Content-relevance video pipeline for a roadmap day.
  // - With a YouTube Data API key: calls search.list with order=relevance
  //   (never order=viewCount) and resolves straight to that video.
  // - Without a key: falls back to YouTube's own relevance-sorted search
  //   results page (default sort = relevance, no view-count sort param).
  const fetchVideoForDay = async (item) => {
    const day = item.day;
    if (videoData[day] && videoData[day].status === 'ready') return; // cache hit

    const query = buildContentQuery(item);
    const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`;

    setVideoData(prev => ({ ...prev, [day]: { status: 'loading', searchUrl } }));

    if (!ytApiKey) {
      // No-key fallback: relevance-sorted YouTube search redirect
      setVideoData(prev => ({
        ...prev,
        [day]: { status: 'ready', mode: 'search', searchUrl, watchUrl: searchUrl, query }
      }));
      return;
    }
    ...
    try {
      const params = new URLSearchParams({
        ...
        q: query,
        type: 'video',
        order: 'relevance', // content relevance, NOT viewCount
        maxResults: '1',
        key: ytApiKey
      });
      ...
      setVideoData(prev => ({
        ...
        [day]: {
          ...
          thumbnail: top.snippet.thumbnails?.medium?.url,
      ...
      console.error('YouTube fetch failed, falling back to search redirect:', err);
      setVideoData(prev => ({
        ...prev,
        [day]: { status: 'ready', mode: 'search', searchUrl, watchUrl: searchUrl, query }
      }));
  ...
  useEffect(() => {
    ...
    const item = currentRoadmap.find(d => d.day === selectedDay);
    if (item) fetchVideoForDay(item);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedDay, course, ytApiKey]);
    ...
    <div style={{ backgroundColor: '#020617', color: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif', padding: '20px', boxSizing: 'border-box', position: 'relative', overflowX: 'hidden' }}>
      ...
      {view === 'landing' && (
        <div style={{ position: 'absolute', top: '25px', right: '35px', zIndex: 30 }}>
          <button onClick={() => setView('plan-input')} style={{ background: 'transparent', border: '1px solid #38bdf8', color: '#38bdf8', padding: '10px 24px', borderRadius: '12px', fontWeight: 'bold', fontSize: '13px', cursor: 'pointer', textShadow: '0 0 10px rgba(56, 189, 248, 0.6)', boxShadow: '0 0 15px rgba(56, 189, 248, 0.2)' }}>
            GET STARTED ➔
          </button>
        </div>
      )}
        ...
        <div style={{ minHeight: '85vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 10 }}>
          ...
          <div style={{ textAlign: 'center', maxWidth: '950px', padding: '0 20px' }}>
            ...
            <div style={{ animation: 'fadeInScale 1s ease-out 1s both' }}>
              <button onClick={() => setView('plan-input')} style={{ background: 'linear-gradient(to right, #6366f1, #06b6d4)', border: 'none', color: '#fff', padding: '16px 42px', borderRadius: '16px', fontWeight: 'bold', fontSize: '14px', cursor: 'pointer', textShadow: '0 2px 5px rgba(0,0,0,0.4)', boxShadow: '0 0 30px rgba(99, 102, 241, 0.6), 0 0 60px rgba(6, 182, 212, 0.3)' }}>
      ...
      {/* VIEW 2: CLEAN CONFIG (no auth — straight from landing) */}
      {view === 'plan-input' && (
        <div style={{ maxWidth: '480px', margin: '40px auto', background: 'rgba(15, 23, 42, 0.85)', backdropFilter: 'blur(16px)', border: '1px solid rgba(56, 189, 248, 0.3)', borderRadius: '24px', padding: '40px', boxShadow: '0 25px 60px -15px rgba(0, 0, 0, 0.8)', animation: 'fadeInScale 0.6s cubic-bezier(0.16, 1, 0.3, 1)', position: 'relative', zIndex: 20 }}>
          ...
          <form onSubmit={handleGenerateRoadmap} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            <div>
              <label style={{ fontSize: '11px', color: '#94a3b8', display: 'block', marginBottom: '6px', textTransform: 'uppercase', fontFamily: 'monospace' }}>YOUTUBE API KEY <span style={{ color: '#64748b', textTransform: 'none' }}>(optional — enables direct relevance-matched video fetch)</span></label>
              <input type="text" value={ytApiKey} onChange={e => setYtApiKey(e.target.value)} placeholder="Leave blank to use relevance-sorted YouTube search" style={{ width: '100%', background: '#020617', border: '1px solid #334155', borderRadius: '12px', padding: '14px', color: '#38bdf8', fontSize: '13px', boxSizing: 'border-box' }} />
            </div>
                ...
                <div style={{ background: '#0f172a', border: '1px solid #38bdf8', borderRadius: '16px', padding: '25px', animation: 'fadeInScale 0.4s ease-out' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div style={{ background: '#020617', padding: '18px', borderRadius: '12px', border: '1px solid #334155' }}>
                      <div style={{ fontSize: '12px', fontWeight: 'bold', color: '#fff', marginBottom: '8px' }}>Content-Matched YouTube Session</div>
                      <p style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '12px' }}>Matched by topic relevance — not by view count.</p>

                      {(!videoData[selectedDay] || videoData[selectedDay].status === 'loading') && (
                        <div style={{ fontSize: '11px', color: '#38bdf8', marginBottom: '10px' }}>🔎 Finding the most relevant video…</div>
                      )}

                      {videoData[selectedDay]?.status === 'ready' && videoData[selectedDay].mode === 'direct' && (
                        <div style={{ marginBottom: '12px' }}>
                          {videoData[selectedDay].thumbnail && (
                            <img src={videoData[selectedDay].thumbnail} alt="video thumbnail" style={{ width: '100%', borderRadius: '8px', marginBottom: '8px', display: 'block' }} />
                          )}
                          <div style={{ fontSize: '11px', color: '#f8fafc', lineHeight: '1.4' }}>{videoData[selectedDay].title}</div>
                        </div>
                      )}

                      {videoData[selectedDay]?.status === 'ready' && videoData[selectedDay].mode === 'search' && (
                        <div style={{ fontSize: '10px', color: '#64748b', marginBottom: '10px' }}>Query: "{videoData[selectedDay].query}"</div>
                      )}

                      <a
                        href={videoData[selectedDay]?.watchUrl || '#'}
                        target="_blank"
                        rel="noreferrer"
                        onClick={(e) => { if (!videoData[selectedDay]?.watchUrl) e.preventDefault(); }}
                        style={{ background: '#ef4444', color: '#fff', padding: '10px 18px', borderRadius: '8px', textDecoration: 'none', fontSize: '11px', fontWeight: 'bold', display: 'inline-block', opacity: videoData[selectedDay]?.watchUrl ? 1 : 0.5 }}
                      >
                        {videoData[selectedDay]?.mode === 'direct' ? 'Watch Best-Matched Video ▶' : 'Open Relevance-Matched Search ▶'}
                      </a>
                    </div>
